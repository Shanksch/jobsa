...
function handleCheckableElement(element: HTMLInputElement, value: string): void {
  ...
  if (element.type === 'radio' && element.name) {
    ...
    if (matchedRadio) {
      ...
      // Dispatch events on the matched radio, not the initial one
      element = matchedRadio; 
    }
  } else {
...
export async function injectAnswers(
  ...
  for (const field of extractedFields) {
    ...
    if (element.tagName.toLowerCase() === 'select') {
      handleSelectElement(element as HTMLSelectElement, String(value));
      triggerFrameworkEvents(element);
    } else if (element.type === 'checkbox' || element.type === 'radio') {
      handleCheckableElement(element as HTMLInputElement, String(value));
      triggerFrameworkEvents(element);
    } else {
