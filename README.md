...
│   └── web-dashboard/         Dashboard SPA (React 18 + Vite 5 + TanStack Query)
├── services/
...
# Terminal 2 — Frontend (dashboard + extension dev servers)
npm run dev
```
...
### Local Dev without Supabase (Docker Fallback)
...
**Phase 1 — In Progress.** Core backend services (resume parsing, RAG pipeline, autofill), dashboard pages (Profile, Resumes, Knowledge Base, Applications), and the Chrome extension are functional. See [project_context.md](file:///d:/developer%20projects/jobsa/project_context.md) for detailed current state.
